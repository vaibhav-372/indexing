...
export default function Game() {
  ...
  const handleRightClick = (boxKey, rowIndex, value, levelData) => {  // Add levelData parameter
    setBoxStates(prev => {
      const boxState = prev[boxKey];
      if (boxState.completedRows[rowIndex]) return prev;
      ...
      let nextRow = (boxState.currentRow + 1) % row; // Always move to next row
      const updatedCompleted = [...boxState.completedRows];
      updatedCompleted[rowIndex] = true; // Always mark as completed

      // Calculate new data index
      const newDataIndex = isMatch
        ? boxState.dataIndex  // Keep same data if matched
        : (boxState.dataIndex + 1) % levelData.col1.length; // Next data if not matched
          ...
          <table className="w-full border-collapse text-sm">
            ...
            <tbody>
              {Array.from({ length: row }, (_, i) => {
                ...
                const dataIndex = boxState.dataIndex % levelData.col1.length;
