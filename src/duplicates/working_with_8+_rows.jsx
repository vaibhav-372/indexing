...
export default function Game() {
  const [row, setRow] = useState(8); // start with 8 rows
      ...
      .then((res) => {
        if (res.data && res.data[boxKey]?.[`level${level}`]) {
          ...
          setBoxStates((prev) => {
            ...
            if (boxState.currentRow < updatedRowData.length) {
              updatedRowData[boxState.currentRow] = {
  ...
  const handleRightClick = (boxKey, rowIndex, value, levelData) => {
    setBoxStates(prev => {
      ...
      let nextDataIndex = isMatch ? currentDataIndex : currentDataIndex + 1;

      let nextRow = boxState.currentRow + 1;

      // ✅ If all rows are completed, extend arrays first
      let newRowAdded = false;
      if (updatedCompleted.every(val => val)) {
        // setRow(prevRow => prevRow + 1); // extend global row count
      ...
      // ✅ If we just added a new row, point currentRow correctly
      if (newRowAdded) {
        nextRow = updatedRowData.length - 1;
        // preload data for new row
      ...
      } else if (!isMatch && nextRow < updatedRowData.length) {
        // only preload if mismatch and row already exists
        updatedRowData[nextRow] = {
          col1: levelData.col1[nextDataIndex % levelData.col1.length] || '',
          col2: levelData.col2[nextDataIndex % levelData.col2.length] || ''
        };
      ...
      <div className="bg-white shadow-lg rounded-2xl overflow-hidden border border-gray-200 w-full">
        ...
        <div className="p-4">
          <table className="w-full border-collapse text-sm">
            ...
            <tbody>
              {Array.from({ length: boxState.rowData.length }, (_, i) => {
                      ...
                      <button
                        ...
                        className={`px-3 py-1 rounded-lg shadow-sm transition ${leftDisabled
                          ? boxState.left[i] === btn1
                            ? 'bg-red-500 text-white' // ✅ highlight chosen left button red
                      ...
                      <button
                        ...
                        className={`px-3 py-1 rounded-lg shadow-sm transition ${rightDisabled
                          ? boxState.right[i] === btn1
                            ? 'bg-red-500 text-white' // ✅ highlight chosen right button green
                            : 'bg-gray-300 text-gray-500 cursor-not-allowed'
                          : boxState.right[i] === btn1
                            ? 'bg-gray-700 text-white'
