...
export default function Game() {
  ...
  // Global sequence index across all boxes (carry-forward)
  ...
  const getDisplayedRowDataFor = (boxKey, rowIdx) => {
    ...
    const levelData = data[boxKey];
    if (rowIdx < bState.rowData.length && bState.rowData[rowIdx] !== null) {
      return bState.rowData[rowIdx] || { col1: '', col2: '' };
    }
    if (
      rowIdx === bState.currentRow &&
      levelData?.col1 && Array.isArray(levelData.col1) &&
      levelData?.col2 && Array.isArray(levelData.col2)
    ) {
      return {
        col1: levelData.col1[globalIndex % levelData.col1.length] || '',
        col2: levelData.col2[globalIndex % levelData.col2.length] || ''
      };
    }
    return { col1: '', col2: '' };
  };
  ...
  const handleRightClick = (boxKey, rowIndex, value, levelData) => {
    ...
    setBoxStates(prev => {
      ...
      let nextGlobal = globalIndex;
      if (boxState.left[rowIndex] !== value) {
        nextGlobal = globalIndex + 1;
      }
      ...
      updatedStates[boxKey] = {
        ...
        dataIndex: boxState.dataIndex, // keep per-box dataIndex stable (not used for carry)
      };
      ...
      if (
        ...
        Object.keys(updatedStates).forEach(otherKey => {
          ...
          const otherCompleted = otherState.completedRows[prevRow];
          const otherIsHyphen = otherState.rowData[prevRow]?.col1 === '-' && otherState.rowData[prevRow]?.col2 === '-';
          if (!otherCompleted && !otherIsHyphen) {
      ...
      // Update lastSameBoxClick and global index for next evaluation
      setLastSameBoxClick({ boxKey, rowIndex });
      if (nextGlobal !== globalIndex) {
            ...
            <tbody>
              ...
              {Array.from({ length: maxRows }, (_, i) => {
                ...
                if (i < boxState.rowData.length && boxState.rowData[i] !== null) {
                  ...
                  // Use global sequence preview for current row
                  rowData = {
                    col1: levelData.col1[globalIndex % levelData.col1.length] || "",
                    col2: levelData.col2[globalIndex % levelData.col2.length] || ""
                  };
